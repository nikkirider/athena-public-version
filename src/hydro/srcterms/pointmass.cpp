...
#pragma omp simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) cons(fluidnum,IEN,k,j,i) -=
          dt*0.5*(pmb->pcoord->phy_src1_i_(i)*flux[X1DIR](fluidnum,IDN,k,j,i)*gm_
                 +pmb->pcoord->phy_src2_i_(i)*flux[X1DIR](fluidnum,IDN,k,j,i+1)*gm_);
      }
    }
  }
  }
