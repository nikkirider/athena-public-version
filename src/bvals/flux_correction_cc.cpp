      ...
      if (nb.fid==INNER_X1 || nb.fid==OUTER_X1) {
        ...
        if (pmb->block_size.nx3>1) { // 3D
          ...
          for (int nn=ns; nn<=ne; nn++) {
            for (int k=pmb->ks; k<=pmb->ke; k+=2) {
              for (int j=pmb->js; j<=pmb->je; j+=2) {
                ...
                sbuf[p++]= (x1flux(fluidnum, nn, k  , j  , i)*amm
                           +x1flux(fluidnum, nn, k  , j+1, i)*amp
                           +x1flux(fluidnum, nn, k+1, j  , i)*apm
                           +x1flux(fluidnum, nn, k+1, j+1, i)*app)/tarea;
              }
            }
          }
          }
          ...
          for (int nn=ns; nn<=ne; nn++) {
            for (int j=pmb->js; j<=pmb->je; j+=2) {
              ...
              Real tarea=am+ap;
              sbuf[p++]=(x1flux(fluidnum, nn, k, j  , i)*am
                        +x1flux(fluidnum, nn, k, j+1, i)*ap)/tarea;
            }
          }
          }
        } else { // 1D
          ...
          for (int fluidnum=0; fluidnum<(NFLUIDS); fluidnum++){
          for (int nn=ns; nn<=ne; nn++){
            sbuf[p++]=x1flux(fluidnum, nn, k, j, i);
          }
          }
        }
        // x2 direction
        ...
        if (pmb->block_size.nx3>1) { // 3D
          ...
          for (int nn=ns; nn<=ne; nn++) {
            for (int k=pmb->ks; k<=pmb->ke; k+=2) {
              ...
              for (int i=pmb->is; i<=pmb->ie; i+=2) {
                ...
                sbuf[p++]=(x2flux(fluidnum, nn, k  , j, i  )*sarea_[0](i  )
                          +x2flux(fluidnum, nn, k  , j, i+1)*sarea_[0](i+1)
                          +x2flux(fluidnum, nn, k+1, j, i  )*sarea_[1](i  )
                          +x2flux(fluidnum, nn, k+1, j, i+1)*sarea_[1](i+1))/tarea;
              }
            }
          }
          }
          ...
          for (int nn=ns; nn<=ne; nn++) {
            ...
            for (int i=pmb->is; i<=pmb->ie; i+=2) {
              ...
              sbuf[p++]=(x2flux(fluidnum, nn, k, j, i  )*sarea_[0](i  )
                        +x2flux(fluidnum, nn, k, j, i+1)*sarea_[0](i+1))/tarea;
            }
          }
          }
        }
        ...
        for (int nn=ns; nn<=ne; nn++) {
          for (int j=pmb->js; j<=pmb->je; j+=2) {
            ...
            for (int i=pmb->is; i<=pmb->ie; i+=2) {
              Real tarea=sarea_[0](i)+sarea_[0](i+1)+sarea_[1](i)+sarea_[1](i+1);
              sbuf[p++]=(x3flux(fluidnum, nn, k, j  , i  )*sarea_[0](i  )
                        +x3flux(fluidnum, nn, k, j  , i+1)*sarea_[0](i+1)
                        +x3flux(fluidnum, nn, k, j+1, i  )*sarea_[1](i  )
                        +x3flux(fluidnum, nn, k, j+1, i+1)*sarea_[1](i+1))/tarea;
            }
          }
        }
        }
    ...
    if (nb.level==pmb->loc.level+1) {
      ...
      if (nb.fid==INNER_X1 || nb.fid==OUTER_X1) {
        ...
        for (int nn=ns; nn<=ne; nn++) {
          for (int k=ks; k<=ke; k++) {
            for (int j=js; j<=je; j++)
              x1flux(fluidnum,nn,k,j,is)=rbuf[p++];
          }
        }
        }
      } else if (nb.fid==INNER_X2 || nb.fid==OUTER_X2) {
        ...
        for (int nn=ns; nn<=ne; nn++) {
          for (int k=ks; k<=ke; k++) {
            for (int i=is; i<=ie; i++)
              x2flux(fluidnum,nn,k,js,i)=rbuf[p++];
          }
        }
        }
      } else if (nb.fid==INNER_X3 || nb.fid==OUTER_X3) {
        ...
        for (int nn=ns; nn<=ne; nn++) {
          for (int j=js; j<=je; j++) {
            for (int i=is; i<=ie; i++)
              x3flux(fluidnum,nn,ks,j,i)=rbuf[p++];
          }
        }
        }
      }
