...
Real WallVel(Real xf, int i, Real time, Real dt, int dir, AthenaArray<Real> gridData) {
  ...
  if (COORDINATE_SYSTEM == "cartesian") {
    ...
    if (time < tST) {
      ...
      myVel =1.1*vej*std::pow(time/tST,-0.6);
    }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real Pej = 0.01*Eej * gm1 /(4/3*M_PI*std::pow(rout-0.5*dr,3));
  Real KE  = 0.99*Eej;
  ...
  for (int i=is; i<=ie; i++) {
    ...
    Real v0  = std::sqrt(15.0*KE/(19.0*M_PI*dej*std::pow(Rej,3.0))); // ramped velocity
    Real vel = 0.0;
    ...
    if (rad < 0.5*Rej) {
      ...
      vel = v0*0.5*(1.0-std::tanh((rad-rout)/dr));
    }
